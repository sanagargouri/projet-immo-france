...
- Environnement : Windows, terminal cmd
...
- `notebooks/01_exploration_dvf.ipynb` — exploration initiale pure (aperçu, comptage, valeurs manquantes,
  ...
  Découpé en deux en cours de route (devenu trop long, 96 cellules).
- `notebooks/02_regles_nettoyage_dvf.ipynb` — vérifications approfondies et décisions de règles
...
- Bug NaN identifié et corrigé : une comparaison `min_val == max_val` classait à tort les
...
- La règle "vente en bloc" (transactions multi-lignes à prix identique) a nécessité plusieurs itérations :
  ...
  trancher un seuil (`nb_lignes <= 9` → agréger, au-delà → exclure) — arbitrage raisonné, la donnée ne
  montrant pas de rupture nette.
- Un outlier extrême (722 590 000€, une Maison) a survécu à toutes les règles jusqu'au bout — transaction
  à ligne unique, donc invisible pour la règle vente en bloc (basée sur le nombre de lignes). Seul le
  seuil outlier final (percentile 99.9) l'a exclu — bon rappel que différentes règles attrapent différents
  types d'anomalies, aucune n'est suffisante seule.
...
Prochaine étape : traduire les 9 règles de `docs/regles_nettoyage.md` en jobs Talend, avec
